...
void receiver(const map<uint32_t, pair<uint32_t, string>> &registered_devices, int reference_id, int sample_count, string output_file, int recording_start_frame, int recording_finish_frame)
{
    // TODO: add assert
    hideCursor();
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        for (int i = 1; i < argc; ++i)
        {
            ...
            else
            {
                ...
                std::cerr << "Usage: .\\Lab [-s <number_of_calibration samples>] [-i <input_file>] [-o <output_file>] [-t <number_of_frames_to_record>] [-r <reference_sensor>] [-f <recording_start_frame>]\n";
                return 1;
            }
        }
    }
    ...
    thread recvThread(receiver, registered_devices, reference_sensor_id, number_of_calibration_samples, output_file, recording_start_frame, recording_finish_frame);
